...
class MorningReportData:
    ...
    generated_at: str = ""
    kr_rows: list[Quote] = field(default_factory=list)  # 야간선물/환율/WTI (검증된 값만)
    us_rows: list[Quote] = field(default_factory=list)  # NASDAQ/S&P500/DOW/SOX
    top_news: list[NewsArticle] = field(default_factory=list)  # TOP 7
    themes: list[dict] = field(default_factory=list)  # [{name, count, sample}]
