...
from config.markets import MORNING_US_INDICES, WTI_SYMBOL
...
def _yahoo(symbol: str) -> tuple[float | None, float | None]:
    ...
    try:
        ...
        if price is None:
            return None, None
        return float(price), (round((price / prev - 1) * 100, 2) if prev else None)
...
def _usdkrw() -> dict | None:
    ...
    price, chg = _yahoo("USDKRW=X")  # 3) Yahoo
    if price is not None:
        return {"price": price, "change_pct": chg, "source": "yahoo"}
    return None


def collect() -> dict[str, dict | None]:
    """{usdkrw, wti, nasdaq, sp500, dow, sox} → raw {price, change_pct, source} | None."""
    global _memo
    if _memo is not None:
        return _memo

    out: dict[str, dict | None] = {"usdkrw": _usdkrw()}
    price, chg = _yahoo(WTI_SYMBOL)
    out["wti"] = {"price": price, "change_pct": chg, "source": "yahoo"} if price is not None else None
    for key, symbol, _label in MORNING_US_INDICES:
        price, chg = _yahoo(symbol)
        out[key] = {"price": price, "change_pct": chg, "source": "yahoo"} if price is not None else None
