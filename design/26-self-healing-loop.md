...
- 스케줄: **금요일 21:00 KST**(= `0 12 * * 5` UTC) — 주말 정지 정책(8-6) 도입 전에는 "주말 저지대"를 노려 일요일이었으나, 주말에 아무것도 돌지 않게 되면서 일요일 실행은 정책과 모순이 됐다. 금요일 저녁은 그 주 시세 배치가 끝난 뒤이자 주말 정적 구간 직전이다
...
- 스케줄: 08:00 KST **월~금** = `0 23 * * 0-4` UTC(전일 23:00) — morning 배치(06:30 KST)와 stock KR 개장 배치 이후라 사람이 아침에 결과를 확인할 수 있다. 주말 제외는 8-6 정책 때문만이 아니라, 주말에 만든 PR은 이틀간 리뷰 없이 방치돼 main과 벌어지기 때문이다
...
### 8-4. 감사 기준·원장·vault 축적 (2026-07-30 추가)
...
### 8-6. 주말 정지 정책 (2026-07-31 도입)

시장이 닫힌 48시간 동안 워크플로가 계속 돌면 의미 없는 커밋과 알림만 쌓인다. 저장소가 PUBLIC이라
Actions 비용은 무료이므로, 정지의 목적은 **비용이 아니라 잡음 제거**다.

**KST 토 09:00 = UTC 토 00:00.** 그래서 "KST 주말"이 UTC 요일 `0,6`과 정확히 일치한다 —
크론에 시간대 보정이 필요 없고, 평일은 그냥 `1-5`다. 이 우연이 설계를 단순하게 만든다.

| 워크플로 | 평일 | 주말 | 근거 |
|---|---|---|---|
| stock | `0 0-6 * * 1-5` · `35 6 * * 1-5` · `0 22 * * 1-5` | 정지 | 이미 월~금. 금요일 미국 종가는 토 07:00 KST 실행이 잡는다(경계 안쪽) |
| morning | `30 21 * * 0-4` | 정지 | 이미 월~금 |
| macro | `0 * * * 1-5` | 정지 | FRED·ECOS 주말 발표 없음. **BTC는 24h지만 주말 갱신을 포기**하는 대가로 잡음을 줄인다 |
| financials | `0 21 * * 0-4` | 정지 | DART·EDGAR 주말 공시 없음 |
| news | `*/30 * * * 1-5` | **`0 */2 * * 0,6`** | 속보는 주말에도 난다 — 정지하지 않고 감속만 |
| health | `15 * * * 1-5` | **`15 */6 * * 0,6`** | 감시견을 끄면 주말 장애를 월요일에야 안다 |
| trades | push 트리거 | 그대로 | 스케줄 없음 |

**센서 임계값을 같이 바꾸지 않으면 정책이 곧 오탐이 된다.** 크론만 고치면 토·일 내내 워커
23종이 "안 돌았다"고 잡힌다. `config/slo.py`의 `max_age_weekend_h`가 이 둘을 잇는 유일한 고리다.

| 상수 | 값 | 공백 계산 |
|---|---|---|
| `_H_WEEKEND_SLOW` | 6h | 주말 뉴스 2시간 주기 — 2회 연속 실패까지 흡수 |
| `_H_WEEKEND_STOP` | 52h | macro 마지막 금 23:00 UTC → 다음 월 00:00 UTC = 49h + 유예 3h |
| `_H_DAY_WEEKEND` | 80h | financials 마지막 목 21:00 UTC → 다음 일 21:00 UTC = 72h + 유예 8h |

유예를 넉넉히 잡은 이유가 있다. 처음 76h로 뒀더니 월요일 재개 직전 여유가 **1.1시간**뿐이었다 —
GitHub Actions의 schedule은 best-effort라 피크에 1~2시간씩 밀리므로 지연 한 번이면 가짜 위반이
난다. 민감도보다 오탐 0이 우선이므로 80h로 넓혔다. 현재 여유는 macro 3.1h · stock 4.1h ·
financials 5.1h · morning 5.6h다.

**정지가 장애를 가리지 않는다.** 신선도(`max_age`) 규칙만 완화되고 상태(`status`)·건수
(`min_items`) 규칙은 주말에도 그대로다. 주말에 워커가 `error`로 떨어지면 즉시 잡힌다 —
회귀 테스트 `test_weekend_does_not_mask_a_real_failure`가 이 계약을 고정한다.

**루틴(Phase E·F)도 같은 정책을 따른다** — Fixer는 월~금(`0 23 * * 0-4`), Auditor는 금요일
(`0 12 * * 5`)로 옮겼다(§3-4·§3-5).
