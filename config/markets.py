...
# 두 소스 등락률 차가 이 값(%p)을 넘으면 degraded로 표시한다.
# 실측(2026-07-24) 기준 지수는 소수점까지 일치했고 VIX만 0.40%p 벌어졌다(기준 종가 차이).
CROSS_CHECK_TOLERANCE_PP = 0.5

# 지수 기준(0.5%p)을 그대로 쓰면 **VIX는 영구히 degraded**가 된다. 실측 2026-08-10:
#   VIX  가격 yahoo 15.43 vs fdr 15.22 (0.21pt = 값의 1.4%) → 등락률 차 1.48%p
#   S&P  가격 7755 수준, 등락률 차 0.00%p / 나스닥·다우 0.15%p 이내
# 같은 **비율**의 가격 불일치가 절대값 ~15인 VIX에서는 등락률로 수십 배 증폭된다.
# 두 소스의 스냅샷 시각이 조금만 어긋나도 벌어지는 값이라, 0.5%p는 VIX에게
# "S&P를 0.07%p로 재는" 것과 같은 비현실적 기준이다.
#
# 허용오차는 그 종목의 **평상시 일변동 폭**에 비례해야 한다 — VIX 일변동 σ는 지수의 5~10배다.
# 항상 켜져 있는 경고는 경고가 아니므로(신호가 아니라 잡음이 된다) 종목별로 나눈다.
# 단, 넓히는 것은 신호를 죽이는 일이기도 하다 — 2%p면 위 관측치(1.48%p)는 통과하되
# 소스 하나가 진짜로 어긋나는 규모(자릿수 밀림·기준가 오배정)는 여전히 잡는다.
CROSS_CHECK_TOLERANCE_PP_BY_KEY: dict[str, float] = {
    "vix": 2.0,
    "move": 2.0,   # MOVE도 절대값 ~70대의 변동성 지수 — 같은 증폭을 받는다
}


def cross_check_tolerance(key: str) -> float:
    """종목별 교차검증 허용오차(%p). 미등재 종목은 지수 기준."""
    return CROSS_CHECK_TOLERANCE_PP_BY_KEY.get(key, CROSS_CHECK_TOLERANCE_PP)
